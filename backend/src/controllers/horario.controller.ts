...
export async function crear(req: Request, res: Response): Promise<void> {
  const horario = await crearHorario(req.user!.usuarioId, req.body);
...
export async function editar(req: Request, res: Response): Promise<void> {
  const horario = await editarHorario(req.user!.usuarioId, req.params.id as string, req.body);
  res.json({ horario });
}

export async function borrar(req: Request, res: Response): Promise<void> {
  await borrarHorario(req.user!.usuarioId, req.params.id as string);
  res.status(204).send();
}
